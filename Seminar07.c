...
Heap initializareHeap(int lungime) {
	...
	heap.nrMasini = 0;
	heap.v = (Masina*)malloc(sizeof(Masina) * lungime);
	return heap;
}
...
void filtreazaHeap(Heap heap, int pozitieNod) {
	...
	if (pozFiuSt<heap.nrMasini&&heap.v[pozMax].pret < heap.v[pozFiuSt].pret)// daca trebuia sa filtram dupa pret inlocuiam doar .id cu .pret
	{
		pozMax = pozFiuSt;
	}
	if (pozFiuDr<heap.nrMasini&&heap.v[pozMax].pret < heap.v[pozFiuDr].pret)
	{
		pozMax = pozFiuDr;
	}
	if (pozMax != pozitieNod)
	{
		Masina aux = heap.v[pozMax];
		heap.v[pozMax] = heap.v[pozitieNod];
		heap.v[pozitieNod] = aux;
	}
	//verificarea ultimului parinte
...
Heap citireHeapDeMasiniDinFisier(const char* numeFisier) {
	...
	while (!feof(file)) {
		heap.v[heap.nrMasini++] = citireMasinaDinFisier(file);
	}
	fclose(file);
...
Masina extrageMasina(Heap* heap) {
	...
	{
		Masina aux = heap->v[0];
		heap->v[0] = heap->v[heap->nrMasini - 1];
		heap->v[heap->nrMasini - 1] = aux;
		heap->nrMasini--;
